...
@push('styles')
<style>
    .participant-list{display:flex;flex-direction:column;gap:9px}.participant-item{min-width:0;border:1px solid var(--line);border-radius:9px;background:#fff;overflow:hidden}.participant-summary{display:grid;grid-template-columns:42px minmax(170px,1.5fr) minmax(130px,1fr) minmax(130px,1fr) auto auto;gap:14px;align-items:center;padding:11px 13px;list-style:none}.participant-item details>.participant-summary{cursor:pointer}.participant-summary::-webkit-details-marker{display:none}.participant-item details>.participant-summary:hover{background:#fafafa}.participant-team{min-width:0}.participant-team strong{display:block}.participant-team small{display:block}.participant-edit{padding:16px;border-top:1px solid var(--line);background:#fafafa}.participant-chevron{color:var(--muted);transition:transform .15s}.participant-item details[open] .participant-chevron{transform:rotate(180deg)}@media(max-width:900px){.participant-summary{grid-template-columns:32px minmax(0,1fr) auto auto;gap:9px;padding:10px}.participant-hide-mobile{display:none}.participant-team strong,.participant-team small{overflow:hidden;white-space:nowrap;text-overflow:ellipsis}}@media(max-width:680px){.participant-edit{padding:14px}.participant-edit .form-grid{grid-template-columns:1fr}}
</style>
@endpush
@section('content')
<div class="page-head">
...
<section class="card">
    ...
    <form class="short-link-form" method="post" action="{{ route('tournaments.share-link.update', $tournament) }}">@csrf @method('PATCH')
        <div><label for="share_slug">{{ __('ui.short_viewer_link') }}</label><div class="short-link-input"><span>{{ url('/view') }}/</span><input id="share_slug" name="share_slug" required minlength="4" maxlength="36" pattern="[a-z0-9](?:[a-z0-9-]*[a-z0-9])" value="{{ old('share_slug', Illuminate\Support\Str::isUuid($tournament->public_token) ? '' : $tournament->public_token) }}" placeholder="{{ __('ui.short_viewer_link_placeholder') }}" aria-label="{{ __('ui.share_slug') }}" autocomplete="off" autocapitalize="none" spellcheck="false"></div><small class="muted">{{ __('ui.short_viewer_link_help') }}</small></div>
        <button class="btn" type="submit" data-submitting="{{ __('ui.saving') }}">{{ __('ui.save_short_link') }}</button>
    </form>
    @else
