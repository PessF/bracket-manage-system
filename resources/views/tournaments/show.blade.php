...
<section class="card">
    ...
    <form class="short-link-form" method="post" action="{{ route('tournaments.share-link.update', $tournament) }}">@csrf @method('PATCH')
        <div><label for="share_slug">{{ __('ui.short_viewer_link') }}</label><div class="short-link-input"><span>{{ url('/view') }}/</span><input id="share_slug" name="share_slug" required minlength="4" maxlength="36" pattern="[a-z0-9](?:[a-z0-9-]*[a-z0-9])" value="{{ old('share_slug', $tournament->public_token) }}" autocomplete="off" autocapitalize="none" spellcheck="false"></div><small class="muted">{{ __('ui.short_viewer_link_help') }}</small></div>
        <button class="btn secondary">{{ __('ui.save_short_link') }}</button>
    </form>
    @else
