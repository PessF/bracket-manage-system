...
@if($isAdmin)
<section class="card">
    <div class="actions" style="justify-content:space-between;margin-bottom:10px"><div><h2 style="margin:0 0 3px">{{ __('ui.share_view_link') }}</h2><div class="muted">{{ __('ui.share_link_help') }}</div></div><span class="badge {{ $tournament->status === App\Enums\TournamentStatus::LIVE ? 'available-now' : $tournament->status->value }}">{{ $tournament->status === App\Enums\TournamentStatus::LIVE ? __('ui.available_now') : __('ui.available_when_live') }}</span></div>
    @if($shareUrl)
