...
<div class="form-grid">
<div class="field full"><label for="event_id">{{ __('events.event') }}</label><select id="event_id" name="event_id" required><option value="">{{ __('events.select') }}</option>@foreach($events as $event)<option value="{{ $event->id }}" @selected(old('event_id', $tournament->event_id) === $event->id)>{{ $event->name }}</option>@endforeach</select><a href="{{ route('events.create') }}">{{ __('events.new') }}</a></div>
