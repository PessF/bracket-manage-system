...
@if(!$isAdmin)<div class="alert view-only-banner"><strong>{{ auth()->check() ? __('ui.viewer_account_active') : __('ui.viewer_mode') }}</strong><div>{{ __('ui.share_only_notice') }}</div></div>@endif
@if($isAdmin)<form class="filter-bar inline-form" method="get"><div class="field"><label for="status">{{ __('ui.status') }}</label><select id="status" name="status"><option value="">{{ __('ui.all_statuses') }}</option>@foreach(App\Enums\TournamentStatus::cases() as $status)<option value="{{ $status->value }}" @selected(request('status') === $status->value)>{{ __('ui.tournament_status_labels.'.$status->value) }}</option>@endforeach</select></div><button class="btn secondary">{{ __('ui.filter') }}</button></form>@endif
