...
<section class="card" id="examples"><h2>{{ __('api.request_examples') }}</h2><h3>{{ __('api.list_example') }}</h3><pre><code>curl "{{ url('/api/tournaments?format=ROUND_ROBIN&search=EasyKids&per_page=20&lang=th') }}"</code></pre><h3>{{ __('api.create_example') }}</h3><pre><code>curl -X POST "{{ url('/api/tournaments') }}" \
  -H "Authorization: Bearer YOUR_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -H "Accept-Language: th-TH" \
  -d '{"name":"EasyKids 2026","competition":"Robot Challenge","division":"Junior","format":"DOUBLE_ELIMINATION","seeding_method":"REGISTRATION_ORDER"}'</code></pre><h3>{{ __('api.status_example') }}</h3><pre><code>curl -X PATCH "{{ url('/api/tournaments/{id}/status') }}" \
  -H "Authorization: Bearer YOUR_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
...
@push('styles')
<style>
.api-layout{display:grid;grid-template-columns:220px minmax(0,1fr);gap:18px;align-items:start}.api-toc{position:sticky;top:78px;display:flex;flex-direction:column;gap:4px}.api-toc strong{margin-bottom:7px}.api-toc a{padding:7px 8px;border-radius:6px;color:var(--muted);font-size:12px}.api-toc a:hover{background:var(--soft);color:var(--ink);opacity:1}.api-content{min-width:0}.api-content h3{margin:22px 0 7px;font-size:13px}.api-content p{color:#52525b}.api-base{display:flex;align-items:center;gap:12px;margin:14px 0 22px;padding:11px;border-radius:8px;background:var(--soft)}.api-base span{font-weight:700}.api-base code{overflow:auto}.api-section-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px}.api-section-head h2{margin-bottom:2px}.api-note{padding:10px 12px;border-left:3px solid #f59e0b;background:#fffbeb;color:#92400e}.api-method{font-weight:800;white-space:nowrap}.api-endpoint{display:block;max-width:560px;overflow:auto;white-space:nowrap}pre{max-width:100%;overflow:auto;padding:14px;border:1px solid #27272a;border-radius:8px;background:#18181b;color:#f4f4f5;font-size:12px}ol{padding-left:22px}ol li+li{margin-top:6px}@media(max-width:860px){.api-layout{grid-template-columns:1fr}.api-toc{position:static;display:none}}@media(max-width:680px){.api-section-head{align-items:flex-start}.api-content .card{margin-right:-2px;margin-left:-2px}.api-endpoint{max-width:68vw}}
</style>
@endpush
